...
class MatrixOperations(Matrix):
    ...
    def add_matrix(cls, first_matrix, second_matrix):
        ...
        return Matrix.make_from_list([[elem1 + elem2 for elem1, elem2 in zip(row1, row2)]
                                      for row1, row2 in
    ...
    def scalar_multiply_matrix(cls, scalar, matrix):
        if scalar == 0:
            ...
            return matrix
        return Matrix.make_from_list([[scalar * elem for elem in row] for row in matrix.data_lines])

    @classmethod
    def transpose(cls, matrix):
        return Matrix.make_from_list([list(row) for row in zip(*matrix.data_lines)])
